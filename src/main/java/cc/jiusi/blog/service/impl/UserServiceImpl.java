...
import cc.jiusi.blog.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     */
    @Override
    public String loginByUsername(LoginDto loginDto, HttpServletRequest request) {
        // 参数校验
    ...
    private void updateLoginInfo(HttpServletRequest request, User user) {
        // 保存登录信息
    ...
    public void sendEmailCode(LoginDto loginDto) {
        ...
        new Thread(() -> {
            try {
                EmailUtil.sendEmailCode(loginDto.getEmail(), code);
                // 保存验证码至redis(5分钟有效)
